...
-- Fund and cash positions on the latest date, excluding near-zero balances.
holdings_latest as (
    select
        fct_hd.account_key,
        fct_hd.fund_key,
    ...
    where (fct_hd.holding_type = 'Cash') or (fct_hd.units_held   >= 0.01)
),
...
valued as (
    select
        ...
        hl.value_gbp,
        case 
          when holding_type = 'Cash' then null
          else greatest(coalesce(bc.total_cost_gbp, 0) - coalesce(sp.total_proceeds_gbp, 0), 0)
        end as cost_basis_gbp
    from holdings_latest hl
    inner join {{ ref('dim_account') }} da  on da.account_key = hl.account_key
    left join {{ ref('dim_fund') }}    df  on df.fund_key    = hl.fund_key
...
select
    ...
    case 
        when holding_type = 'Cash' then null
        else round(value_gbp - cost_basis_gbp, 2)
    end                                                                                   as unrealised_gain_gbp,
    case
        when holding_type = 'Cash' then null
        when cost_basis_gbp > 0 then round((value_gbp - cost_basis_gbp) / cost_basis_gbp * 100.0, 2)
        else 0.0
    end                                                                                   as unrealised_gain_pct,
    round(value_gbp / sum(value_gbp) over (partition by account_name) * 100.0, 2)         as weight_pct

from valued
order by account_name, holding_type, fund_name desc
